...
export function Skills({ skills }: { skills: SkillsState }) {
  const counts = {
  ...
  return (
    <section>
      <SectionHead
        title="Skills"
        count={`${skills.installed_count} · ${skills.total_invocations} invocations · ${skills.window_days}d`}
      />
      ...
      <div className="bg-white rounded-lg border border-zinc-200 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-100">
            {all.map((r, i) => (
              <tr key={`${r.skill}-${i}`} className="hover:bg-zinc-50">
                <td className="px-4 py-2.5">
                  <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium border ${BADGE[r._c]}`}>
                    {r._c}
                  </span>
                </td>
                <td className="px-4 py-2.5 mono text-zinc-900">{r.skill}</td>
                <td className={`px-4 py-2.5 text-right mono ${r.invocations ? "text-zinc-900 font-medium" : "text-zinc-400"}`}>
                  {r.invocations}
                </td>
